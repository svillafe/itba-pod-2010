...
/**
 * Market implementation using a local thread. This market implementaiton uses a
 * thread to look for possible matches, and join buyers and sellers
 */
@ThreadSafe
public class LocalMarket extends CleanableThread implements Market,
		MarketInspector {
	protected final Multiset<ResourceStock> selling = ConcurrentHashMultiset
			.create();
	protected final Multiset<ResourceStock> buying = ConcurrentHashMultiset
			.create();
	...
	public Collection<ResourceDemand> managedResources() {
		...
		for (Resource res : resources) {
			ResourceDemand demand = ResourceDemand.on(res)
					.needed(needed.count(res)).offered(offered.count(res))
					.build();
			result.add(demand);
		}
		return result;
	...
	public Collection<BidInfo> bidsFor(Resource resource) {
		...
		for (Multiset.Entry<ResourceStock> entry : selling.entrySet()) {
			info.add(BidInfo.forSell(entry.getElement().name(), entry
					.getElement().resource(), entry.getCount()));
		}
		for (Multiset.Entry<ResourceStock> entry : buying.entrySet()) {
			info.add(BidInfo.forBuy(entry.getElement().name(), entry
					.getElement().resource(), entry.getCount()));
		}
		return info;
	}
			...
			if (procured) {
				...
				if (sent) {
					...
					try {
						...
						// TODO: To fully solve this case, 2PCommit or 3PC is
	 ...
	 *            transfered
	 */
	protected void createhistory(ResourceStock from, ResourceStock to,
			int amount) {
	 ...
	 *            transfered
	 */
	protected void createHistory(String fromName, String toName,
	...
	/**
	 * @param resourceSet
	 *            the resource to count
	 * @return
	 */
	private Multiset<Resource> getResourceSet(
			Multiset<ResourceStock> resourceSet) {
